...
// TODO: add start mode, start/end registers
...
async function testIt(modeHandler: ModeHandler, testParams: ITestParams): Promise<void> {
  ...
  // Find the cursors and take them out of the start/end strings
  ...
  if (testParams.jumps !== undefined) {
    ...
    const actualJumpPosition =
      ...
      '<FRONT>';
    const expectedJumpPosition = stripBar(testParams.jumps.find(t => t.includes('|'))) || '<FRONT>';
